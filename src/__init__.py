...
class TransducerFile:
    ...
    def __init__(self, filename:Path | str, search_cutoff:int=60):
        ...
        if not (self.transducer.get_type() in [hfst.ImplementationType.HFST_OL_TYPE, hfst.ImplementationType.HFST_OLW_TYPE]):
    ...
    def lookup_lemma_with_affixes(self, surface_form: str) -> list[Analysis]:
        return [analysis for weight, analysis in self.weighted_lookup_lemma_with_affixes(surface_form)]
    ...
    def lookup_symbols(self, input: str) -> list[list[str]]:
        ...
        return [
            transduction
            for weight, transduction in self.weighted_lookup_symbols(input)
        ]    
    ...
    def weighted_lookup_symbols(self, input: str) -> list[tuple[float,list[str]]]:
        ...
        """
        return [
            (float(weight), [symbol for symbol in symbols if not hfst.is_diacritic(symbol) and symbol])
